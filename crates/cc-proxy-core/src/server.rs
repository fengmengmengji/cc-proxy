...
async fn create_message(
    ...
    // Try Responses path first (if detected); on upstream error, silently
    // fall back to ChatCompletions. Many OpenAI-compatible relays have
    // transient 500s on /responses, so this auto-fallback keeps the user
    // experience seamless.
    if state.api_mode == upstream::mode::UpstreamApiMode::Responses {
        let result = try_responses_path(
            &state,
            &request,
            is_stream,
            first_byte_timeout,
        ...
        match result {
            ...
            Err(e) => {
                tracing::warn!(
                    "Responses path failed ({}), falling back to ChatCompletions",
                    e
                );
            }
        }
    ...
    // ChatCompletions path (primary when detected, or fallback from Responses).
    let openai_request = convert::request::claude_to_openai(&request, &state.config);
    let chat_tool_map = crate::util::tool_name::build_map(request.tools.as_deref());
...
/// Attempt the Responses API path; returns the axum response on success.
async fn try_responses_path(
    state: &Arc<AppState>,
    ...
    tool_name_map: crate::util::tool_name::ToolNameMap,
